...
class DetailWeatherInfoViewController: UIViewController {
    ...
    private func getWeatherDetailInfo(row : Int)-> (detailText:String, nameOfImage:String, infoName:String) {
        var dataText = ""
        var imageName = ""
        var infoName = ""
        ...
        return (dataText, imageName, infoName)
    }
...
extension DetailWeatherInfoViewController: DetailInfoViewModelProtocal {
    ...
    func didUpdateDetailsWeatherInfo() {
        ...
        let weatherIcon:String = (detailInfoViewModel.weatherDeatilsInfoList?.weather[0].icon)!
        let url = URL(string: "http://openweathermap.org/img/w/\(weatherIcon).png")!
